...
export default function ConfiguracoesPage() {
  const { user } = useAuth();
  const isAdmin = ['ADMIN', 'SUPER_ADMIN'].includes((user as DynamicValue)?.role || '');
  ...
  async function atualizar(u: Usuario, data: Partial<Usuario>) {
    ...
    const d = await res.json().catch(() => ({}));
    if (!res.ok) {
      setMsg({ type: 'err', text: d.error || 'Erro ao atualizar' });
      return;
    }
    await load();
  }
    ...
    <div className="mx-auto w-full max-w-[1100px] space-y-5 pb-8">
      <div>
        <div className="mb-2 text-xs font-medium text-slate-500">
          RIGOR <span className="text-rigor-orange">›</span> Configurações
        </div>
        <h1 className="rigor-title text-3xl leading-none">Configurações</h1>
      ...
      {isAdmin && (
        <div className="rigor-card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="rigor-title flex items-center gap-2 text-xl">
              <Users className="h-5 w-5 text-rigor-orange" />
              Usuários do sistema
            </h2>
            <Button size="sm" className="rigor-btn-primary" onClick={() => setShowForm(true)}>
                  ...
                  {usuarios.map((u) => (
                    <tr key={u.id} className="border-b last:border-0 hover:bg-slate-50">
                      <td className="px-3 py-3 font-semibold">
                        {u.name}
                        {u.id === (user as DynamicValue)?.id && (
                          <span className="ml-2 rounded bg-orange-50 px-1.5 py-0.5 text-[10px] font-bold text-rigor-orange">
                            você
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <select
                          value={u.role}
                          onChange={(e) => atualizar(u, { role: e.target.value })}
                          className="h-8 rounded-md border border-slate-200 bg-white px-2 text-xs font-semibold outline-none focus:border-rigor-orange"
                        >
                          {ROLES.map((r) => (
                      ...
                      <td className="px-3 py-3 text-right">
                        <button
                          onClick={() => atualizar(u, { isActive: !u.isActive })}
                          className={cn(
      ...
      <div className="rigor-card max-w-xl p-5">
        <h2 className="rigor-title mb-4 flex items-center gap-2 text-xl">
          <KeyRound className="h-5 w-5 text-rigor-orange" />
          Alterar minha senha
        </h2>
      ...
      {showForm && (
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h3 className="rigor-title text-xl">Novo usuário</h3>
              <button
                onClick={() => setShowForm(false)}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="block text-sm font-bold">
                  ...
                  <select
                    ...
                    className="mt-1 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                  >
                    {ROLES.map((r) => (
                      <option key={r} value={r}>
                        {roleLabel[r]}
