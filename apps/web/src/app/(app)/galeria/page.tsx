      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-2 text-xs font-medium text-slate-500">
            RIGOR <span className="text-rigor-orange">›</span> Galeria
          </div>
          <h1 className="rigor-title text-3xl leading-none">Galeria</h1>
      ...
      {loading ? (
        ...
        <div className="rigor-card py-20 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Selecione uma imagem do computador para iniciar a galeria.
          </p>
        </div>
      ) : (
        Object.entries(grupos).map(([mes, items]) => (
          <div key={mes}>
            <h2 className="rigor-title mb-3 text-xl capitalize">
              {mes} <span className="text-sm font-normal text-slate-400">({items.length})</span>
            </h2>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
              {items.map((f) => (
                <button
                  ...
                  className="group relative aspect-[4/3] overflow-hidden rounded-xl border bg-slate-100 text-left"
                >
            ...
            <div className="mt-3 flex items-center justify-between rounded-lg bg-white/10 p-3 text-white">
              <div>
                ...
                <p className="text-xs text-white/70">
                  {new Date(lightbox.data).toLocaleString('pt-BR')}
                  {lightbox.etapa ? ` · ${lightbox.etapa.nome}` : ''}
                  {lightbox.tags ? ` · ${lightbox.tags}` : ''}
                </p>
              </div>
          ...
          >
            ...
            <form onSubmit={salvarFoto} className="space-y-4">
              ...
              <label className="block text-sm font-bold">
                ...
                <input
                  name="tags"
                  placeholder="fundacao, antes, deposito..."
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
