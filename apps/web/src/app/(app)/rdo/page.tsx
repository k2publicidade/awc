  ...
  return (
    <div className="h-full w-full">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="font-['Arial_Narrow',Arial,sans-serif] text-[31px] font-black uppercase leading-none tracking-[-.04em] text-[#17212b]">
            RDO
          </h1>
          <p className="mt-2 text-[13px] text-[#66717d]">
            Relatórios diários de obra — clima, efetivo, atividades e ocorrências
          </p>
        </div>
        <Link href="/rdo/novo">
        ...
        <div className="relative h-[47px] w-full max-w-[260px]">
          ...
          {search && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-[#6f7984] hover:bg-slate-100 hover:text-[#17212b]"
            >
              <X className="h-4 w-4" />
            </button>
          )}
      ...
      <div className="overflow-hidden rounded-[6px] border border-[#e1e6eb] bg-white shadow-[0_10px_30px_rgba(23,33,43,.06)]">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="font-bold text-[#26323d]">
              {search ? `Nenhum RDO encontrado para “${search}”` : 'Nenhum RDO encontrado'}
            </p>
            <p className="mt-1 text-[13px] text-[#68727d]">
              {search
                ? 'Ajuste o termo ou limpe a busca para ver todos os relatórios.'
                : 'Clique em “Novo RDO” para criar o primeiro relatório diário.'}
            </p>
            {search && (
              <Button
                variant="outline"
                className="mt-4 rounded-[5px] font-bold"
                onClick={() => {
                  setSearch('');
                  setPage(1);
                }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-[13px]">
              ...
              <tbody>
                {rdos.map((rdo: DynamicValue, i: number) => (
                  <tr
                    key={rdo.id}
                    className="border-b border-[#edf0f3] last:border-0 hover:bg-[#fafbfc]"
                  >
                    <td className="px-5 py-5 font-bold text-[#27323e]">
                    ...
                    <td className="px-3 py-5 text-[#24303b]">
                      {rdo.data ? new Date(rdo.data).toLocaleDateString('pt-BR') : '—'}
                    </td>
                    <td className="px-3 py-5 text-[#24303b]">#{rdo.numero || '—'}</td>
                    <td className="px-3 py-5 text-[#24303b]">{rdo.responsavel?.name || '—'}</td>
                    <td className="px-3 py-5 text-[#24303b]">
...
function RdoPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#e4e9ee] bg-[#fbfcfd] px-5 py-3">
      <p className="text-[12px] font-semibold text-[#64707c]">
      ...
      <nav className="flex items-center gap-1" aria-label="Paginação">
        <button
          ...
          className={cn(btn, 'text-[#52606d] hover:bg-slate-100')}
        >
          <ChevronLeft className="h-4 w-4" />
        </button>
        <span className="px-2 text-[12px] font-bold text-[#26323d]">
          {page} / {pages}
        </span>
        <button
          disabled={page === pages}
  ...
  return (
    <span
      className={cn(
        ...
        isRascunho ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'
      )}
    >
      <span
        className={cn('h-1.5 w-1.5 rounded-full', isRascunho ? 'bg-amber-500' : 'bg-green-500')}
      />
