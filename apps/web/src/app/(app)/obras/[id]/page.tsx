    ...
    <div className="mx-auto w-full max-w-[1540px] space-y-5 pb-8">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <Link
            href="/obras"
            className="mb-2 inline-flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-rigor-orange"
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            Obras
          </Link>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="rigor-title text-3xl leading-none">{obra.nome}</h1>
            <span className={cn('inline-flex rounded-md px-2.5 py-1 text-xs font-bold', st.cls)}>
              {st.label}
            </span>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <a
            ...
            target="_blank"
            className="inline-flex h-9 items-center rounded-md border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50"
          >
          ...
          <Link
            href="/rdo/novo"
            className="inline-flex h-9 items-center rounded-md bg-gradient-to-br from-[#ff4d00] to-[#ff7a1a] px-4 text-sm font-semibold text-white shadow-[0_10px_22px_rgba(255,77,0,.22)] hover:brightness-95"
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="rigor-card p-4">
          ...
          <p className="mt-1 text-2xl font-bold text-rigor-orange">{avanco.toFixed(0)}%</p>
          <div className="mt-2 h-2 w-full rounded-full bg-slate-100">
            <div
              className="h-2 rounded-full bg-rigor-orange"
        ...
        <div className="rigor-card p-4">
          ...
          <p className="mt-1 text-2xl font-bold text-rigor-dark">
            {money(Number(obra.valorContratado))}
          </p>
          <p className="mt-1 text-xs text-slate-500">
            {obra.dataInicio
              ? `Início ${obra.dataInicio.toLocaleDateString('pt-BR')}`
              : 'Início não definido'}
          </p>
        </div>
        <div className="rigor-card p-4">
          ...
          <p
            className={cn(
              ...
              receitas - despesas >= 0 ? 'text-green-700' : 'text-rigor-danger'
            )}
          >
            {money(receitas - despesas)}
          </p>
          <p className="mt-1 text-xs text-slate-500">
            Receitas {money(receitas)} · Despesas {money(despesas)}
          </p>
        </div>
        <div className="rigor-card p-4">
          ...
          <p
            className={cn(
              'mt-1 text-2xl font-bold',
              etapasAtrasadas.length > 0 ? 'text-rigor-danger' : 'text-green-700'
            )}
          >
          ...
          <p className="mt-1 text-xs text-slate-500">
            {etapasAtrasadas.length > 0
              ? `${etapasAtrasadas.length} etapa(s) atrasada(s)`
              : 'Sem etapas atrasadas'}
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
        <div className="rigor-card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="rigor-title text-xl">Cronograma</h2>
            <Link href="/cronograma" className="text-sm font-semibold text-rigor-orange">
              Abrir Gantt
            </Link>
          </div>
          {obra.etapas.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-500">
              ...
              <Link href="/cronograma" className="font-semibold text-rigor-orange">
                Monte o cronograma
              </Link>
              .
            </p>
                ...
                return (
                  <div key={e.id} className="flex items-center gap-3">
                    ...
                    <span className="w-10 text-right text-xs font-bold">
                      {(e.percentualRealizado || 0).toFixed(0)}%
                    </span>
                    {atrasada && <AlertTriangle className="h-4 w-4 text-rigor-danger" />}
                  </div>
                );
              })}
            </div>
        ...
        <div className="space-y-4">
          <div className="rigor-card p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="rigor-title text-xl">Últimos RDOs</h2>
              <Link href="/rdo" className="text-sm font-semibold text-rigor-orange">
                Ver todos
              </Link>
            </div>
            {obra.rdos.length === 0 ? (
              ...
              <div className="space-y-2 text-sm">
                {obra.rdos.map((r) => (
                  <Link
                    ...
                    className="flex items-center justify-between rounded-lg border p-2.5 hover:bg-slate-50"
                  >
          ...
          <div className="rigor-card p-5">
            <h2 className="rigor-title mb-3 text-xl">Atalhos</h2>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <Link
                href="/financeiro"
                className="flex items-center gap-2 rounded-lg border p-3 hover:bg-slate-50"
              >
                <DollarSign className="h-4 w-4 text-rigor-orange" />
                Financeiro
              </Link>
              <Link
                href="/documentos"
                className="flex items-center gap-2 rounded-lg border p-3 hover:bg-slate-50"
              >
                <FileText className="h-4 w-4 text-rigor-orange" />
              ...
              <Link
                ...
                className="flex items-center gap-2 rounded-lg border p-3 hover:bg-slate-50"
              >
                <ShieldCheck className="h-4 w-4 text-rigor-orange" />
                Qualidade
              </Link>
              <Link
                href="/andamento"
                className="flex items-center gap-2 rounded-lg border p-3 hover:bg-slate-50"
              >
                <BarChart3 className="h-4 w-4 text-rigor-orange" />
                Andamento
              </Link>
            </div>
          </div>
          ...
          {obra.ocorrencias.length > 0 && (
            <div className="rigor-card p-5">
              <h2 className="rigor-title mb-3 text-xl">Ocorrências abertas</h2>
              <div className="space-y-2 text-sm">
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {o.tipo.replaceAll('_', ' ')} · {o.dataAbertura.toLocaleDateString('pt-BR')}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          {obra.medicoes.length > 0 && (
            <div className="rigor-card p-5">
              <h2 className="rigor-title mb-3 text-xl">Medições</h2>
              <div className="space-y-2 text-sm">
                {obra.medicoes.map((m) => (
                  <div
                    key={m.id}
      ...
      {obra.descricao && (
        <div className="rigor-card p-5">
          <h2 className="rigor-title mb-2 text-xl">Descrição</h2>
          <p className="text-sm text-slate-600">{obra.descricao}</p>
        </div>
      )}

      <div className="rigor-card p-5">
        <h2 className="rigor-title mb-3 flex items-center gap-2 text-xl">
          <CalendarDays className="h-5 w-5 text-rigor-orange" />
          Datas
        </h2>
        <div className="grid gap-3 text-sm md:grid-cols-3">
          <div className="rounded-lg border p-3">
            <p className="text-xs font-bold uppercase text-slate-500">Início</p>
            <p className="mt-1 font-semibold">
              {obra.dataInicio?.toLocaleDateString('pt-BR') || '—'}
            </p>
          </div>
          <div className="rounded-lg border p-3">
            <p className="text-xs font-bold uppercase text-slate-500">Previsão de término</p>
            <p className="mt-1 font-semibold">
          ...
          <div className="rounded-lg border p-3">
            ...
            <p className="mt-1 font-semibold">
              {obra.dataConclusao?.toLocaleDateString('pt-BR') || 'Em andamento'}
            </p>
          </div>
        </div>
      </div>
