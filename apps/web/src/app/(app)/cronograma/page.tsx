...
export default function CronogramaPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      if (obraId) void loadCronograma(obraId);
    }, 0);
    return () => window.clearTimeout(timer);
  }, [obraId, loadCronograma]);

  async function updateEtapa(id: string, data: DynamicValue) {
    setError('');
    ...
    if (!res.ok) {
      ...
      setError(d.error || 'Erro ao atualizar etapa');
      return;
    }
  ...
  async function removeEtapa(etapa: Etapa) {
    ...
    if (!res.ok) {
      ...
      toast({
        ...
        description: d.error || 'Tente novamente.',
      });
      return;
    }
  ...
  async function saveEtapa(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/cronograma/etapa/${editEtapa!.id}` : '/api/crud/etapas',
        {
      ...
      toast({
        ...
        description: `"${payload.nome}" salva com sucesso.`,
      });
      await loadCronograma(obraId);
  ...
  const avancoMedio = etapas.length
    ? etapas.reduce((s, e) => s + (e.percentualRealizado || 0), 0) / etapas.length
    : 0;
  const atrasadas = etapas.filter(
    (e) => e.dataFim && new Date(e.dataFim) < new Date() && (e.percentualRealizado || 0) < 100
  ).length;
    ...
    <div className="mx-auto w-full max-w-[1540px] space-y-5 pb-8">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-2 text-xs font-medium text-slate-500">
            RIGOR <span className="text-rigor-orange">›</span> Cronograma
          </div>
          <h1 className="rigor-title text-3xl leading-none">Cronograma</h1>
          <p className="mt-1 text-sm text-slate-500">
            Etapas, prazos, avanço físico e baseline por obra
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            disabled={!obraId || saving || etapas.length === 0}
          >
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rigor-card p-4">
          <p className="text-xs font-bold uppercase text-slate-500">Etapas</p>
          <p className="mt-1 text-2xl font-bold text-rigor-orange">{etapas.length}</p>
        </div>
        <div className="rigor-card p-4">
          <p className="text-xs font-bold uppercase text-slate-500">Avanço médio</p>
          <p className="mt-1 text-2xl font-bold text-rigor-dark">{avancoMedio.toFixed(0)}%</p>
        </div>
        <div className="rigor-card p-4">
          <p className="text-xs font-bold uppercase text-slate-500">Etapas atrasadas</p>
          <p
            className={cn(
              'mt-1 text-2xl font-bold',
              atrasadas > 0 ? 'text-rigor-danger' : 'text-green-700'
            )}
          >
            {atrasadas}
          </p>
        ...
        <div className="rigor-card p-4">
          <p className="text-xs font-bold uppercase text-slate-500">Valor planejado</p>
      ...
      <div className="rigor-card overflow-x-auto p-5">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Clique em “Nova etapa” para montar o cronograma.
            </p>
          </div>
        ) : (
          <GanttChart
      ...
      {!loading && etapas.length > 0 && (
        <div className="rigor-card p-5">
          <h2 className="rigor-title mb-4 text-xl">Etapas</h2>
              ...
              <tbody>
                {etapas.map((e) => (
                  <tr key={e.id} className="border-b last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-slate-600">
                      {e.dataFim ? new Date(e.dataFim).toLocaleDateString('pt-BR') : '—'}
                    </td>
                    ...
                    <td className="px-3 py-3">
                      {Number(e.valorFinanceiro || 0).toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL',
                        maximumFractionDigits: 0,
                      })}
                    ...
                    <td className="px-3 py-3 text-right">
                      <button
                        ...
                        className="mr-2 rounded p-1.5 text-rigor-orange hover:bg-orange-50 text-xs font-bold"
                      >
                        Editar
                      </button>
                      <button
      ...
      {versoes.length > 0 && (
        <div className="rigor-card p-5">
          <h2 className="rigor-title mb-4 text-xl">Baselines salvos</h2>
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h3 className="rigor-title text-xl">{editEtapa ? 'Editar etapa' : 'Nova etapa'}</h3>
              <button
                onClick={() => {
                  setShowForm(false);
                  setEditEtapa(null);
            ...
            <form onSubmit={saveEtapa} className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block text-sm font-bold">
                ...
                <input
                  ...
                  type="date"
                  defaultValue={
                    editEtapa?.dataInicio
                      ? new Date(editEtapa.dataInicio).toISOString().slice(0, 10)
                      : ''
                  }
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              ...
              <label className="block text-sm font-bold">
                ...
                <input
                  ...
                  type="date"
                  defaultValue={
                    editEtapa?.dataFim ? new Date(editEtapa.dataFim).toISOString().slice(0, 10) : ''
                  }
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
              <label className="block text-sm font-bold">
                % Previsto
                <input
                  name="percentualPrevisto"
                  type="number"
                  min={0}
                  max={100}
                  defaultValue={editEtapa?.percentualPrevisto ?? 0}
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
              <label className="block text-sm font-bold">
                ...
                <input
                  ...
                  min={0}
                  max={100}
                  defaultValue={editEtapa?.percentualRealizado ?? 0}
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
              <label className="block text-sm font-bold">
                ...
                <input
                  ...
                  step="0.01"
                  min={0}
                  defaultValue={Number(editEtapa?.valorFinanceiro || 0)}
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
              <label className="block text-sm font-bold">
                ...
                <input
                  ...
                  type="number"
                  min={1}
                  defaultValue={editEtapa?.ordem ?? etapas.length + 1}
                  className="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm font-normal outline-none focus:border-rigor-orange"
                />
              </label>
