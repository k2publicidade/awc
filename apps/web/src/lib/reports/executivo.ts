...
import { wrapReport, formatCurrency, progressBar, statusBadge, RIGOR_COLORS } from './pdf-generator';
...
export async function generateRelatorioExecutivo(obraId: string): Promise<string> {
  ...
  const etapasAtrasadas = obra.etapas.filter(
    (e) => e.dataFim && e.dataFim < new Date() && (e.percentualRealizado || 0) < 100
  ).length;
  ...
  const content = `
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Despesas</div><div class="metric-value" style="font-size:14px;color:${RIGOR_COLORS.danger}">${formatCurrency(despesas)}</div></div>
      <div class="metric-card"><div class="metric-label">Receitas</div><div class="metric-value" style="font-size:14px;color:${RIGOR_COLORS.success}">${formatCurrency(receitas)}</div></div>
      <div class="metric-card"><div class="metric-label">Margem</div><div class="metric-value" style="font-size:14px;color:${receitas - despesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(receitas - despesas)}</div></div>
    </div>
    ...
    <table>
      ...
      <tr><td style="font-weight:600">Etapas Atrasadas</td><td>${statusBadge(etapasAtrasadas > 0 ? RIGOR_COLORS.danger : RIGOR_COLORS.success, `${etapasAtrasadas} atrasada(s)`)}</td></tr>
      <tr><td style="font-weight:600">Ocorrências Abertas</td><td>${statusBadge(ncsAbertas > 0 ? RIGOR_COLORS.warning : RIGOR_COLORS.success, `${ncsAbertas} aberta(s)`)}</td></tr>
    </table>
    ...
    <table>
      ...
      <tr><td style="font-weight:600">Saldo</td><td style="color:${Number(orcamento.valorTotal) - despesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(Number(orcamento.valorTotal) - despesas)}</td></tr>
    </table>`
        : ''
    }
    ...
    <table>
      ...
      <tbody>
        ${obra.rdos.map((r) => `<tr><td>${r.data?.toLocaleDateString('pt-BR')}</td><td>${r.climaManha || '-'}</td><td>${r.assinaturaNome || '-'}</td><td>${(r as DynamicValue).atividades?.length || 0}</td></tr>`).join('')}
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ${obra.medicoes.map((m) => `<tr><td>${m.periodoInicio?.toLocaleDateString('pt-BR')} — ${m.periodoFim?.toLocaleDateString('pt-BR')}</td><td>${formatCurrency(m.valorTotal)}</td><td>${statusBadge(m.status.startsWith('APROVADA') ? RIGOR_COLORS.success : RIGOR_COLORS.warning, m.status)}</td></tr>`).join('')}
      </tbody>
    </table>`
        : ''
    }
