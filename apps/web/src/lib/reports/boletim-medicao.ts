...
import { wrapReport, formatCurrency, statusBadge, RIGOR_COLORS } from './pdf-generator';
...
export async function generateBoletimMedicao(medicaoId: string): Promise<string> {
  ...
  const content = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Status</div><div class="metric-value">${statusBadge(medicao.status.startsWith('APROVADA') ? RIGOR_COLORS.success : RIGOR_COLORS.warning, medicao.status)}</div></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        ${(medicao.itens || [])
          ...
          .join('')}
        <tr style="background:${RIGOR_COLORS.dark};color:#fff">
          <td colspan="2" style="text-align:right;font-weight:700;color:#fff">TOTAL</td>
          <td colspan="2" style="font-weight:700;color:${RIGOR_COLORS.orange}">${formatCurrency(medicao.valorTotal)}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div style="margin-top:40px;display:flex;justify-content:space-between">
      <div style="text-align:center;width:45%">
        <div style="border-top:1px solid #000;padding-top:8px;margin-top:40px">
          ...
          <p style="font-size:9px;color:${RIGOR_COLORS.gray}">CREA: ______________</p>
        </div>
      </div>
      <div style="text-align:center;width:45%">
        <div style="border-top:1px solid #000;padding-top:8px;margin-top:40px">
          <p style="font-size:10px;font-weight:600">Cliente</p>
          <p style="font-size:9px;color:${RIGOR_COLORS.gray}">Data: ___/___/___</p>
        </div>
      </div>
    </div>

    <div style="margin-top:24px;text-align:center">
      <p style="font-size:8px;color:${RIGOR_COLORS.gray}">Este boletim pode ser verificado via QR Code no sistema RIGOR</p>
    </div>
  `;
