...
import { wrapReport, statusBadge, RIGOR_COLORS } from './pdf-generator';
...
export async function generateRelatorioQualidade(obraId: string): Promise<string> {
  ...
  const itensConformes = inspecoes.reduce(
    (s, i) => s + (i.itens?.filter((it: DynamicValue) => it.resultado === 'CONFORME').length || 0),
    0
  );
  ...
  const content = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Índice Conformidade</div><div class="metric-value" style="color:${indiceNC < 10 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${(100 - indiceNC).toFixed(1)}%</div></div>
      <div class="metric-card"><div class="metric-label">NCs Abertas</div><div class="metric-value" style="color:${ncsAbertas > 0 ? RIGOR_COLORS.danger : RIGOR_COLORS.success}">${ncsAbertas}</div></div>
    ...
    <table>
      ...
      <tbody>
        ${inspecoes
          .map((i) => {
            const conformes =
              i.itens?.filter((it: DynamicValue) => it.resultado === 'CONFORME').length || 0;
            const total = i.itens?.length || 0;
            const resultado = conformes === total ? 'APROVADA' : 'REPROVADA';
            return `<tr><td>${i.data?.toLocaleDateString('pt-BR')}</td><td>${i.etapa?.nome || '-'}</td><td>${i.responsavel?.name || '-'}</td><td>${total}</td><td>${conformes}</td><td>${statusBadge(resultado === 'APROVADA' ? RIGOR_COLORS.success : RIGOR_COLORS.danger, resultado)}</td></tr>`;
          })
          .join('')}
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ${ncs
          .map(
            (nc) => `<tr>
          <td>${(nc as DynamicValue).descricao || '-'}</td>
          <td>${statusBadge((nc as DynamicValue).severidade === 'CRITICO' ? RIGOR_COLORS.danger : (nc as DynamicValue).severidade === 'ALTO' ? RIGOR_COLORS.warning : RIGOR_COLORS.info, (nc as DynamicValue).severidade || '-')}</td>
          <td>${(nc as DynamicValue).causaRaiz || '-'}</td>
          <td>${statusBadge(nc.status === 'ENCERRADA' ? RIGOR_COLORS.success : nc.status === 'EM_EXECUCAO' ? RIGOR_COLORS.warning : RIGOR_COLORS.danger, nc.status)}</td>
          <td>${(nc as DynamicValue).prazo?.toLocaleDateString('pt-BR') || '-'}</td>
        </tr>`
          )
          .join('')}
      </tbody>
