...
import { wrapReport, formatCurrency, RIGOR_COLORS } from './pdf-generator';
...
export async function generateRelatorioFinanceiro(obraId: string): Promise<string> {
  ...
  const content = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Receitas</div><div class="metric-value" style="font-size:14px;color:${RIGOR_COLORS.success}">${formatCurrency(totalReceitas)}</div></div>
      <div class="metric-card"><div class="metric-label">Despesas</div><div class="metric-value" style="font-size:14px;color:${RIGOR_COLORS.danger}">${formatCurrency(totalDespesas)}</div></div>
      <div class="metric-card"><div class="metric-label">Resultado</div><div class="metric-value" style="font-size:14px;color:${totalReceitas - totalDespesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(totalReceitas - totalDespesas)}</div></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        ...
        ${Object.entries(byCategory)
          ...
          .map(
            ([cat, val]) =>
              `<tr><td style="padding-left:24px">${cat}</td><td style="text-align:right;color:${RIGOR_COLORS.danger}">- ${formatCurrency(val)}</td></tr>`
          )
          .join('')}
        <tr style="background:${RIGOR_COLORS.dark}"><td style="font-weight:700;color:#fff">Resultado Líquido</td><td style="text-align:right;font-weight:700;color:${totalReceitas - totalDespesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(totalReceitas - totalDespesas)}</td></tr>
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ${Object.entries(byMonth)
          ...
          .map(
            ([mes, vals]) =>
              `<tr><td>${mes}</td><td style="text-align:right;color:${RIGOR_COLORS.success}">${formatCurrency(vals.receitas)}</td><td style="text-align:right;color:${RIGOR_COLORS.danger}">${formatCurrency(vals.despesas)}</td><td style="text-align:right;font-weight:600;color:${vals.receitas - vals.despesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(vals.receitas - vals.despesas)}</td></tr>`
          )
          .join('')}
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ${Object.entries(byCategory)
          ...
          .map(
            ([cat, val]) =>
              `<tr><td>${cat}</td><td style="text-align:right">${formatCurrency(val)}</td><td style="text-align:right">${totalDespesas > 0 ? ((val / totalDespesas) * 100).toFixed(1) : 0}%</td></tr>`
          )
          .join('')}
      </tbody>
    </table>
    ...
    <table>
      ...
      <tr><td style="font-weight:600">Saldo Orçamentário</td><td style="color:${Number(orc.valorTotal) - totalDespesas >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${formatCurrency(Number(orc.valorTotal) - totalDespesas)}</td></tr>
    </table>`
        : ''
    }
  `;
