...
export async function generateCurvaS(obraId: string): Promise<string> {
  ...
  const totalValor =
    obra.etapas.reduce((s, e) => s + Number(e.valorFinanceiro || 0), 0) ||
    Number(obra.valorContratado);
  const totalRealizado = obra.etapas.reduce(
    (s, e) => s + (Number(e.valorFinanceiro || 0) * (e.percentualRealizado || 0)) / 100,
    0
  );
  ...
  const content = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Valor Realizado</div><div class="metric-value" style="font-size:14px;color:${RIGOR_COLORS.success}">${formatCurrency(totalRealizado)}</div></div>
      <div class="metric-card"><div class="metric-label">Desvio</div><div class="metric-value" style="font-size:14px;color:${totalRealizado >= prevAcum * 0.9 ? RIGOR_COLORS.success : RIGOR_COLORS.danger}">${((totalRealizado / totalValor) * 100).toFixed(1)}%</div></div>
    </div>
      ...
      <svg width="${chartWidth}" height="${chartHeight}" viewBox="0 0 ${chartWidth} ${chartHeight}">
        ...
        ${Array.from({ length: 5 }, (_, i) => {
          ...
          return `<line x1="40" y1="${y}" x2="${chartWidth - 10}" y2="${y}" stroke="#E5E7EB" stroke-width="0.5"/>
                  <text x="35" y="${y + 3}" text-anchor="end" font-size="8" fill="${RIGOR_COLORS.gray}">${i * 25}%</text>`;
        }).join('')}

        <!-- Previsto line -->
        <polyline fill="none" stroke="${RIGOR_COLORS.info}" stroke-width="2"
          points="${semanas.map((s, i) => `${40 + (i * (chartWidth - 50)) / numPontos},${chartHeight - (s.previstoAcum / maxVal) * (chartHeight - 30) - 10}`).join(' ')}" />

        <!-- Realizado line -->
        <polyline fill="none" stroke="${RIGOR_COLORS.orange}" stroke-width="2" stroke-dasharray="5,3"
          points="${semanas.map((s, i) => `${40 + (i * (chartWidth - 50)) / numPontos},${chartHeight - (s.realizadoAcum / maxVal) * (chartHeight - 30) - 10}`).join(' ')}" />

        <!-- Legend -->
        <line x1="200" y1="${chartHeight - 5}" x2="220" y2="${chartHeight - 5}" stroke="${RIGOR_COLORS.info}" stroke-width="2"/>
        <text x="225" y="${chartHeight - 2}" font-size="9" fill="${RIGOR_COLORS.gray}">Previsto</text>
        <line x1="300" y1="${chartHeight - 5}" x2="320" y2="${chartHeight - 5}" stroke="${RIGOR_COLORS.orange}" stroke-width="2" stroke-dasharray="5,3"/>
        <text x="325" y="${chartHeight - 2}" font-size="9" fill="${RIGOR_COLORS.gray}">Realizado</text>
      </svg>
    </div>
    ...
    <table>
      ...
      <tbody>
        ${semanas
          ...
          <td style="color:${desvio >= 0 ? RIGOR_COLORS.success : RIGOR_COLORS.danger};font-weight:600">${desvio >= 0 ? '+' : ''}${formatCurrency(desvio)}</td></tr>`;
          })
          .join('')}
      </tbody>
    </table>
