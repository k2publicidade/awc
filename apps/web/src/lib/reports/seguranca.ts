...
export async function generateRelatorioSeguranca(obraId: string): Promise<string> {
  ...
  const acidentesComAfast = acidentes.filter(
    (a) => (a as DynamicValue).tipo === 'COM_AFASTAMENTO'
  ).length;
  ...
  const content = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:20px">
      ...
      <div class="metric-card"><div class="metric-label">Acidentes</div><div class="metric-value" style="color:${acidentes.length > 0 ? RIGOR_COLORS.danger : RIGOR_COLORS.success}">${acidentes.length}</div></div>
      <div class="metric-card"><div class="metric-label">Taxa de Frequência</div><div class="metric-value">${freqRate.toFixed(2)}</div></div>
      <div class="metric-card"><div class="metric-label">NCs Segurança</div><div class="metric-value" style="color:${ncsSeg.length > 0 ? RIGOR_COLORS.danger : RIGOR_COLORS.success}">${ncsSeg.length}</div></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        ${acidentes.map((a) => `<tr><td>${a.dataHora?.toLocaleDateString('pt-BR')}</td><td>${statusBadge(a.tipo === 'COM_AFASTAMENTO' ? RIGOR_COLORS.danger : RIGOR_COLORS.warning, a.tipo || '-')}</td><td>${a.descricao || '-'}</td><td>${a.vitima?.nome || '-'}</td></tr>`).join('')}
      </tbody>
    </table>`
        : "<p style='color:#22C55E;font-weight:600;margin:16px 0'>✅ Nenhum acidente registrado nesta obra</p>"
    }
    ...
    <table>
      ...
      <tbody>${ncsSeg.map((nc) => `<tr><td>${(nc as DynamicValue).descricao}</td><td>${statusBadge(RIGOR_COLORS.danger, (nc as DynamicValue).severidade)}</td><td>${(nc as DynamicValue).prazo?.toLocaleDateString('pt-BR') || '-'}</td></tr>`).join('')}</tbody>
    </table>`
        : ''
    }
  `;
