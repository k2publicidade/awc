...
import { wrapReport, RIGOR_COLORS } from './pdf-generator';

export async function generateRDOCompilado(
  obraId: string,
  dataInicio: string,
  dataFim: string
): Promise<string> {
    ...
    ${rdos
      ...
      <div style="page-break-inside:avoid;margin-bottom:16px;border:1px solid #E5E7EB;border-radius:6px;padding:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <span style="font-size:10px;color:${RIGOR_COLORS.gray}">☀️ ${rdo.climaManha || '-'} / ${rdo.climaTarde || '-'}</span>
        </div>
        ...
        ${rdo.ocorrenciasRdo.length > 0 ? `<p style="font-size:10px;margin-top:4px;color:${RIGOR_COLORS.danger}"><strong>Ocorrências:</strong> ${rdo.ocorrenciasRdo.map((o) => o.descricao).join('; ')}</p>` : ''}
      </div>`;
      })
      .join('')}
