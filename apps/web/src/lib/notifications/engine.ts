...
export async function checkAllTriggers() {
  ...
  const etapasAtrasadas = await prisma.etapa.findMany({
    ...
    include: { obra: { select: { nome: true, engenheiroId: true, tenantId: true } } },
  });
  for (const etapa of etapasAtrasadas) {
    ...
    await createNotification({
      ...
      tipo: 'ETAPA_ATRASADA',
      userId: etapa.obra.engenheiroId,
      obraId: etapa.obraId,
  ...
  for (const mat of materiais) {
    ...
    if (estoqueAtual <= (mat.estoqueMinimo || 0)) {
      const almoxarifes = await prisma.user.findMany({
        where: { tenantId: mat.tenantId, role: 'ALMOXARIFE', isActive: true },
        take: 1,
      });
  ...
  for (const nc of ncsCriticas) {
    const engs = await prisma.user.findMany({
      where: { tenantId: nc.obra.tenantId, role: 'ENGENHEIRO', isActive: true },
      take: 1,
    });
    if (engs.length > 0) {
      await createNotification({
  ...
  for (const ac of acidentes) {
    const gestores = await prisma.user.findMany({
      where: {
        tenantId: ac.obra.tenantId,
        role: { in: ['ENGENHEIRO', 'ADMIN', 'SUPER_ADMIN'] },
        isActive: true,
      },
    });
  ...
  const exames = await prisma.exameMedico.findMany({
    ...
    include: { trabalhador: { select: { nome: true, tenantId: true } } },
  });
  for (const ex of exames) {
    ...
    if ([30, 15, 0].includes(diff)) {
      const engs = await prisma.user.findMany({
        where: { tenantId: ex.trabalhador.tenantId, role: 'ENGENHEIRO', isActive: true },
        take: 1,
      });
      if (engs.length > 0) {
        await createNotification({
  ...
  for (const obra of obrasComOrc) {
    const orc = await prisma.orcamento.findFirst({
      where: { obraId: obra.id, status: 'APROVADO' },
    });
    if (!orc) continue;
    const gasto =
      obra.lancamentos
        ?.filter((l: DynamicValue) => l.tipo === 'DESPESA')
        .reduce((s: number, l: DynamicValue) => s + Number(l.valor || 0), 0) || 0;
  ...
  const medicoesPendentes = await prisma.medicao.findMany({
    ...
    include: { obra: { select: { nome: true, tenantId: true } } },
  });
  for (const med of medicoesPendentes) {
    const fins = await prisma.user.findMany({
      where: { tenantId: med.obra.tenantId, role: 'FINANCEIRO', isActive: true },
      take: 1,
    });
    if (fins.length > 0) {
      await createNotification({
...
async function createNotification(payload: NotificationPayload) {
  ...
  if (payload.obraId) {
    const obra = await prisma.obra.findFirst({
      where: { id: payload.obraId, tenantId: user.tenantId },
      select: { id: true },
    });
    if (!obra) return;
  }
  ...
  for (const canal of payload.canais) {
    try {
      ...
      console.error(`Dispatch ${canal} failed:`, e);
    }
  }
}
...
async function dispatchEmail(payload: NotificationPayload) {
  ...
  await fetch(apiUrl, {
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM || 'RIGOR <onboarding@resend.dev>',
      to: user.email,
      subject: `[RIGOR] ${payload.titulo}`,
      html: `<div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto">
        <div style="background:#1E2832;padding:20px;text-align:center">
          <h1 style="color:#FF6B00;margin:0;font-size:24px;letter-spacing:2px">RIGOR</h1>
        </div>
...
async function dispatchWhatsApp(payload: NotificationPayload) {
  ...
  const instance = process.env.EVOLUTION_INSTANCE || 'rigor';
